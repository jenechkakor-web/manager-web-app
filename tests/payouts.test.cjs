...
test('HTTP: права, валидация, идемпотентность, параллельные выплаты и сохранность договоров', async t => {
  ...
  const remove = (id,cookie=adminCookie,extra={}) => fetch(`${base}/api/payouts`,{method:'DELETE',headers:{cookie,'Content-Type':'application/json',...extra},body:JSON.stringify({id})});
  assert.equal((await remove(entry.requestId,managerCookie)).status,403);
  assert.equal((await remove(entry.requestId,adminCookie,{Origin:'https://example.com'})).status,403);
  assert.equal((await remove('sale:A1')).status,404);
  ...
  assert.equal((await remove(entry.requestId)).status,200);
  assert.equal((await remove(ledger.find(e=>e.kind==='payment').id)).status,200);
  assert.equal((await remove('deal:A1')).status,200);
  const afterDelete=await (await fetch(`${base}/api/payouts`,{headers:{cookie:managerCookie}})).json();
  assert.equal(afterDelete.totals.revenue,10000,'Deleting bonus preserves revenue');
  ...
  assert(!afterDelete.entries.some(e=>e.id===entry.requestId||e.id==='deal:A1'));
  assert.equal(await fs.readFile(contractsFile,'utf8'),initial,'Deletion preserves contracts');
  assert.equal(JSON.parse(await fs.readFile(path.join(directory,'bonus-ledger.json'),'utf8')).length,7,'Ledger stays recoverable');
  const deletions=JSON.parse(await fs.readFile(path.join(directory,'payout-deletions.json'),'utf8'));
  assert.equal(deletions.length,3,'Repeated deletion is idempotent');
  assert(deletions.every(e=>e.deletedBy===1 && e.snapshot && e.deletedAt));
  assert.equal((await fetch(`${base}/.data/bonus-ledger.json`)).status,403);
  const unchanged=record({number:'PENDING',registryMeta:{...record().registryMeta,paymentStatus:'Предоплата',prepayment:4000},bonusQualifiedAt:''});
  await fs.writeFile(contractsFile,JSON.stringify([unchanged]));
  const update=await fetch(`${base}/api/contracts-registry`,{method:'POST',headers:{cookie:managerCookie,'Content-Type':'application/json'},body:JSON.stringify({action:'update-meta',number:'PENDING',fields:{paymentStatus:'Да'}})});
  assert.equal(update.status,200);
  const saved=JSON.parse(await fs.readFile(contractsFile,'utf8'))[0];
  assert(saved.bonusQualifiedAt);
  assert.equal(saved.registryMeta.prepayment,10000);
  assert.equal(saved.registryMeta.prepaymentOverridden,true);
  assert.equal(saved.amount-saved.registryMeta.prepayment,0);
