...
(async()=>{
  ...
  try {
    ...
    await page.waitForFunction(()=>document.querySelector('#registryStatus')?.textContent.includes('Загружено сделок:'));
    assert.equal(await page.locator('#registryMonthFilter').inputValue(), new Intl.DateTimeFormat('sv-SE',{timeZone:'Europe/Moscow'}).format(new Date()).slice(0,7));
    await page.locator('#registryMonthFilter').selectOption('');
    ...
    await page.waitForFunction(()=>document.querySelector('#registryStatus')?.textContent.includes('Загружено сделок:'));
    await page.locator('#registryMonthFilter').selectOption('');await page.locator('#registryTableBody tr').first().waitFor();
