...
test('Б24 HTTP: ФИО, создание/изменение, повторные события, изоляция и сбои API', async () => {
  ...
  try {
    ...
    for(const user of users.slice(1)) assert.equal((await request('users',{login:user.login,fullName:user.fullName,phone:'+7 (999) 123-45-67',email:user.login+'@example.test',password:'TestPassword2026'},admin)).status,201);
    ...
    assert.equal(accounts.find(u=>u.id===2).email,'anton@example.test');
    assert.equal(accounts.find(u=>u.id===2).phone,'+7 (999) 123-45-67');
    const profilePatch=patch=>request('users',{action:'profile',id:2,...patch},admin,'PUT');
    ...
    const ownProfile=(await (await request('auth/session',undefined,anton,'GET')).json()).user;
    assert.equal(ownProfile.fullName,'Антон Исаков');assert.equal(ownProfile.email,'updated@example.test');assert.equal(ownProfile.phone,'+7 999 765-43-21');
    assert.equal((await request('users',undefined,anton,'GET')).status,403);
    assert.equal((await profilePatch({email:'',phone:''})).status,200);
    assert.equal((await (await request('auth/session',undefined,anton,'GET')).json()).user.email,'');
