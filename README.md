...
В основном приложении кнопка `Справочник` открывает отдельную страницу `library.html`.
Чтобы сохранить изменения для всех:

1. Открыть справочник в приложении.
2. Войти под логином `admin` и паролем `admin2026`.
3. Заполнить или изменить группу, подгруппу, название шаблона и текст описания.
4. Вставить GitHub token с доступом `Contents: Read and write` к репозиторию `jenechkakor-web/manager-web-app`.
5. Нажать `Сохранить`.
